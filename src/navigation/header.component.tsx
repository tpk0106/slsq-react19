...
const Header = () => {
  return (
    <>
      ...
      <nav
        id="mobileMenu"
        className="hidden md:hidden lg:hidden w-full text-2xl z-50 absolute justify-between rounded-[14px] top-40 shadow1-2xl font-roboto"
      >
        <ul>
          {navbarData.map((menu) => {
            return (
              <div
                className="cursor-pointer text-xs bg-black text-[#a855f7] text-center justify-around hover:text-[#fff]"
                key={menu.label}
              >
                <Menu
                  label={menu.label}
